...
!       write (6, *) ' ZBQLU01: X, BINV = ', X, BINV
      ...
      if (ZBQLU01.lt.0.0) then
         write (6, *) ' Warning: uniform random number ZBQLU01 < 0,',
     &                ' X, B2 = ', X, B2
      elseif (ZBQLU01.gt.1.0) then
         write (6, *) ' Warning: uniform random number ZBQLU01 > 1,',
     &                ' X, B2 = ', X, B2
c
c JWMar2019 Temporary fix to stop propagation of errors when ZBQLU01 > 1 in current version
c
         ZBQLU01 = ZBQLU01 - 1.0
      endif
...
*
      ...
      DOUBLE PRECISION SPARE, temp_U, ran1
      INTEGER STATUS
      integer *4 idum1
...
!      write (6, *) 'ZBQLNOR 1, MU, SIGMA = ', MU, SIGMA
      IF (STATUS.EQ.-1) PI = 4.0D0*DATAN(1.0D0)

      IF (STATUS.LE.0) THEN
!       write (6, *) 'ZBQLNOR 2'
!JW Mar2019 Use ran1 routine for uniform random number generator because of issues currently with ZBQLU01
!       temp_U = ZBQLU01(0.0D0)
       temp_U = ran1 (idum1)
       THETA = 2.0D0*PI*temp_U
!       write (6, *) 'ZBQLNOR 2a, THETA, temp_U = ', THETA, temp_U
!JW Mar2019 Use ran1 routine for uniform random number generator because of issues currently with ZBQLU01
!       temp_U = ZBQLU01(0.0D0)
       temp_U = ran1 (idum1)
       R = DSQRT( -2.0D0*DLOG(temp_U) )
!       write (6, *) 'ZBQLNOR 2b, R, temp_U = ', R, temp_U
       ZBQLNOR = (R*DCOS(THETA))
       SPARE = (R*DSIN(THETA))
       STATUS = 1
!       write (6, *) 'ZBQLNOR 3, ZBLQNOR, SPARE = ', ZBQLNOR, SPARE
      ELSE
       ZBQLNOR = SPARE
       STATUS = 0
!       write (6, *) 'ZBQLNOR 4'
