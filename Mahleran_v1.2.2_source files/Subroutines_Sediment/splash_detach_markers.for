...
c  splashed and entrained
c
c  JC May 2011
c
c****************************************************************

       subroutine splash_detach_markers
...
c      Determine mass of splashed soil in all cells for each phi class (kg)
       do k = 1, 6
        do im = 1, nr1
            do jm = 1, nc1
c               Convert detach_soil (mm) from output in splash_transport to give total detached soil (kg)
...
c               Then convert into a density (kg/m3)
     ...
     &              (k, im, jm) * (dx_m ** 2)
c               Finaly convert into mass kg) density * 1.0d3 is needed to convert g/cm3 to kg/m3
                detach_soil_m (k, im, jm) = detach_soil_m
...
c      Estimate probability of marker being splashed. See Wright (1987) Equation 1
c      Available soil depth for detachment assumed to be equal to one grain diameter
     ...
     &    .eq.0.0d0) then
        p_splash = 0.0d0
          ...
          p_splash = (detach_tot_m (mycell, mxcell) / ((density * 1.0d3)
